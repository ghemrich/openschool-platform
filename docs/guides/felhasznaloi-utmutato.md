...
A négy admin oldal között fülszerű navigáció biztosítja a váltást (Áttekintés / Felhasználók / Kurzusok / Előléptetés).
...
### Előléptetési szabályok (`/admin/promotion`)

Az előléptetési szabályok automatikusan változtatják a felhasználók szerepkörét, ha a meghatározott kurzusok tanúsítványait megszerezték.

#### Aktív szabályok listája

Minden szabályhoz egy kártya a következő információkkal:
- **Név** és státusz (Aktív / Inaktív badge)
- **Cél szerepkör** (pl. mentor)
- **Szükséges kurzusok** listája
- **Bekapcsolás / Kikapcsolás** gomb — `PATCH /api/admin/promotion-rules/{id}` (`is_active` módosítás)
- **Törlés** gomb — `DELETE /api/admin/promotion-rules/{id}` — megerősítő dialógust kér

#### Új szabály létrehozása

Űrlap:
- **Név** (kötelező) — pl. „Összes kurzus → Mentor"
- **Leírás** (opcionális)
- **Cél szerepkör** — legördülő lista: `mentor` / `admin`
- **Szükséges kurzusok** — jelölőnégyzetek az összes meglévő kurzushoz + „Összes kijelölése" link
- **„Szabály létrehozása"** gomb → `POST /api/admin/promotion-rules`

**Működés:** Amikor egy tanuló tanúsítványt igényel (`POST /api/me/courses/{id}/certificate`), a rendszer automatikusan ellenőrzi az összes aktív előléptetési szabályt. Ha a tanuló rendelkezik a szabályban meghatározott összes kurzus tanúsítványával, automatikusan előlép a cél szerepkörre. Az előléptetéskor Discord értesítés és szerepkör szinkronizáció is történik.
